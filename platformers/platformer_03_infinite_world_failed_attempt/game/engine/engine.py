...
class GameEngine:
    ...
    @staticmethod
    def check_collision_sides(item_a: pg.Rect, item_b: pg.Rect, margin: int = 5) -> dict:
        ...
        return {
            'top': (abs(item_a.bottom) - abs(item_b.top)) <= margin,
            'right': ((abs(item_a.left)) - abs(item_b.right)) <= margin,
            'bottom': (abs(item_a.top) - abs(item_b.bottom)) >= margin,
            'left': ((abs(item_a.right)) - abs(item_b.left)) <= margin,
