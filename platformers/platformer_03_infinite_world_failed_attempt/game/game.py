...
from .application import WORLD_MAP
...
class Game:
    ...
    def __init__(self):
        ...
        self.player = Player(self.background, self.WORLD_GRAVITY, self.WORLD_GRAVITY_MAX, self.camera)
    ...
    def game_tick(self) -> None:
        ...
        self.camera_move()
        # Move player
        self.player.movement_process(tiles_with_collision)
        # Render + Paint Player
        self.player.render()

        # Debug
        # get font object
        if self.DEBUG_STATS:  # Debug game stats
            font = pygame.font.SysFont('Arial', 15, bold=True)
            # render a given font into an image
            img = font.render(f'Camera X = {self.camera[0]} Camera Y = {self.camera[1]}', True, pygame.Color("red"))
            self.background.blit(img, ((self.background.get_width() - self.background.get_width()) / 2,
                                       (self.background.get_height() - self.background.get_height()) / 2))
    ...
    def camera_move(self):
        ...
        self.camera[0] += (((self.player.rect.x - self.camera[0] - (
                    (self.WIN_WIDTH / 2) + (self.player.DEFAULT_SIZE[0] / 2)))) + self.player.speed) / self.camera_momentum
        # self.camera[1] += ((self.player.rect.y - self.camera[1] - (self.WIN_HEIGHT / 2 )) + self.player.gravity_force_applied)
        # self.camera[1] += ((self.player.rect.y - self.camera[1] - 450) + self.player.gravity_force_applied)
        self.camera[1] += (self.player.rect.y - self.camera[1])
        print(self.camera)
        if self.camera[0] < 0:
            self.camera[0] = 0
        if self.camera[1] < 0:
    ...
    def render_world(self):
        ...
        def make_chunk(y: int, row: list, x: int, chunk_index: int) -> pg.draw.rect:
            ...
            block = pg.draw.rect(self.background, color,
                                 pg.Rect(((x * chunk) - camera_x, (y * chunk) - camera_y, chunk, chunk)))
